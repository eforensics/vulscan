...
class DecodeControl():
    ...
    def FlateDecode(cls, stream):
        try :
            ...
            if ord( stream[0] ) == 13 and ord( stream[1] ) == 10 :
                stream = stream[2:]
            
            if ord( stream[-2] ) == 13 and ord( stream[-1] ) == 10 :
                stream = stream[:-2]
