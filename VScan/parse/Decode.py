...
class CDecode():
	@classmethod
	def fnParamFlateDecode(cls, sHeader):
		# [IN]	sHeader
		# [OUT]
		#	- [SUCCESS]	l_Param
		#	- [FAILURE] False
		...
		try :
			l_Param = []
			l_Options = ["Color", "BitPerComponent", "Columns"]
			for sOption in l_Options :
				sPattern = ".{0,100}?/%s\s([0-9]{1,8})" % sOption
				l_Param_tmp = re.findall( sPattern, sHeader, re.DOTALL )
				if l_Param_tmp != [] :
					l_Param.append( l_Param_tmp[0] )
		except :
			print format_exc()
			return None
		...
		return l_Param
	@classmethod
	def fnParamLZWDecode(cls, sHeader):
		# [IN]	sHeader
		# [OUT]
		#	- [SUCCESS]	l_Param
		#	- [FAILURE] False
		#	- [ERROR]	None
		try :
			return cls.fnParamFlateDecode(sHeader)
		except :
			print format_exc()
			return None
	@classmethod
	def fnParamCCITTFaxDecode(cls, sHeader):
		# [IN]	sHeader
		# [OUT]
		#	- [SUCCESS]	l_Param
		#	- [FAILURE] False
		...
		return l_Param
	@classmethod
	def fnParamDCTDecode(cls, sHeader):
		# [IN]	sHeader
		# [OUT]
		#	- [SUCCESS]	l_Param
		#	- [FAILURE] False
	...
	def fnFlateDecode(cls, s_buffer, l_Param):
        ...
        #    - [ERROR] None
		try :
			return decompress(s_buffer)
		except :
			return False
	@classmethod
	def fnASCII85Decode(cls, s_buffer, l_Param=None):
		...
		return s_outbuffer
	@classmethod
	def fnASCIIHexDecode(cls, s_buffer, l_Param=None):
        # [IN] s_buffer
        #    - Parameter : No
        # [OUT] s_outbuffer
        #    - [SUCCESS] Decoded string data
        ...
        #    - [ERROR] None
		try :
			return binascii.unhexlify(''.join([c for c in s_buffer if c not in ' \t\n\r']).rstrip('>'))
		except :
			return False
	@classmethod
	def fnRunLengthDecode(cls, s_buffer, l_Param=None):
        # [IN] s_buffer
        #    - Parameter : No
        # [OUT] s_outbuffer
        #    - [SUCCESS] Decoded string data
	...
	def fnLZWDecode(cls, s_buffer, l_Param):
		...
		try :
			...
			return ''.join(LZWDecoder(StringIO(s_buffer)).run())
		except :
			return False
